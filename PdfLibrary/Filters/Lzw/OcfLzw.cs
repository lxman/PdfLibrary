...
namespace PdfLibrary.Filters.Lzw
{
    ...
    /// I've made use of its NBitStream for reading the variable bit-widths from the stream.
    ...
    public class OcfLzw2
    {
        ...
        /// </summary>
        internal const int ClearTable = 256;
        ...
        /// </summary>
        internal const int MinChunkSize = 9;


        /// <summary>
        /// The maximum size (in bits) of a compressed chunk
        /// </summary>
        internal const int MaxChunkSize = 13;

        /// <summary>
        /// The first code in the set
        /// </summary>
        internal const int FirstCode = 258;

        //internal const int DICTIONARY_SIZE = 9029;

        internal const int CodeSize = 9; // Default code size ...

        /// <summary>
        /// Expected table size
        /// </summary>
        internal const int TableSize = 1 << MaxChunkSize;
        ...
        {
            var input = new NBitStream(compressedInput);
            ...
            while ((nextCommand = input.Read()) != Eod)
            {
                if (nextCommand < 0)
                    break; // EOF?
                // Do our reset
                if (nextCommand == ClearTable)
                {
                    input.BitsInChunk = MinChunkSize;
                    table = new LzwTable(TableSize);
                    lastCommand = -1;
                }
                else
                {
