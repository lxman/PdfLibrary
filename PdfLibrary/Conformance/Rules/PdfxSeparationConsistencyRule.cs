...
/// <summary>
/// PDF/X-4 separation consistency (ISO 15930-7:2010; ISO 32000-1 8.6.6.4): all Separation colour spaces in a
/// file that name the same colorant must agree on their alternate colour space and tint transform, so a
/// colorant renders identically wherever it appears. The special <c>/None</c> and <c>/All</c> colorants are
/// universal and exempt. A colorant is inconsistent when two of its definitions differ in alternate device
/// family (<see cref="ColourSpaceClassifier"/>) or in tint-transform content
        ...
        foreach ((string colorant, HashSet<string> signatures) in signaturesByColorant)
            if (signatures.Count > 1)
                yield return new Finding
                {
                    ...
                    Clause = ConformanceClauses.For(context.Target, "separation consistency"),
                    Message = $"The separation colorant '{colorant}' is defined with an inconsistent alternate "
                              + "space or tint transform across the file (PDF/X-4).",
                };
    }
