...
public sealed class PdfOutlineItem
{
    ...
    /// <summary>The destination this item navigates to, or null if none/unresolvable. Resolves an
    /// explicit <c>/Dest</c>, a GoTo action's <c>/D</c> (<c>/A</c>), or a named destination — see
    /// <see cref="DestinationResolution"/>.</summary>
    public PdfDestination? Destination
    {
        get => DestinationResolution.Destination(_document, Node.Dict);
        set
        {
            if (value is null)
            {
