...
**Progress 2026-07-18** (font-program slice 1): **31-011** (7.21.4.1-3, a used glyph absent from the embedded program) now maps to `font-program` — the new tri-state simple-font code→GID resolver reports glyph-present (veraPDF 6.2.11.4.1 t2 / 7.21.4.1 t2) for simple TrueType and embedded-charset CFF fonts. The same resolver extends **31-030** (7.21.8-1, `.notdef` reference) from Type0-only to simple fonts (its `font-program` mark already stood). Symbolic fonts, classic Type1, predefined-charset CFF and Type0-non-identity route to `Unknown` (no finding) to hold 0 false positives corpus-wide. CP 31 → 18/29. **Caveat added 2026-08-16 (Task 10 fix round):** on embedded-charset CFF specifically, a code whose name was DERIVED from the encoding's own Unicode value (rather than assigned by `/Differences`, a base-encoding table, or the font's built-in encoding) also routes to `Unknown` — a name-KEYED charset lookup cannot treat this engine's own reverse-AGL reconstruction as authoritative. The TrueType arm is unaffected (it only uses the name as a courier for the encoding's Unicode value before keying the cmap directly, so provenance carries no information there); this claim holds for TrueType as stated.
...
| 28-005 | M | 7.18.1-4 | A form field does not have a TU entry and does not | ua-annotation |
...
| 31-007 | M | 7.21.3.3-1 | The WMode entry in a CMap dictionary is not | `font-dictionary` |
| 31-008 | M | 7.21.3.3-2 | A CMap references another CMap which is not | `font-dictionary` |
