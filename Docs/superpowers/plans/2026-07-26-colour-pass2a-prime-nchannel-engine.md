...
**Goal:** Make the engine's NChannel per-component data *correct* — accept an ICCBased CMYK/Gray process space, build each component's tint ramp from its own `/Colorants` Separation, and stop the page colorant inventory registering a named process colorant as a spot plane — while moving no *corpus* output. (Corrected post-merge, whole-branch review: `PageColorant.Kind` and `TintRamp` are **not** unconsumed — they already feed Pellucid's shipped `SpotColorantRegistry`/`CorpusRenderHash`. See Global Constraints below.)

**Architecture:** Three independent corrections to code Pass 2a already shipped. `ProcessSpaceName` (returns a family-name string, which is what forces the ICCBased rejection) becomes `ProcessChannelCount` (returns the channel count, the question the caller actually has). `BuildTintRamp` prefers `/Attributes /Colorants /<name>` over zeroing the other inputs of the whole-space transform. `PageColorantReader` classifies via the per-component `Role` that Pass 2a already computes, instead of by name alone. This does reach a live consumer today (Pellucid's `SpotColorantRegistry`/`CorpusRenderHash`) — Pass 2b additionally consumes `ColourantComponent`/`Role`, which is new.
...
- **CORRECTED (whole-branch review, post-merge):** this pass does **not** change no rendered output. `PageColorant.Kind` and `TintRamp` already feed a live, shipped consumer today — `Pellucid.Rendering.Cmyk.SpotColorantRegistry.Build` (keys on `Kind`) and `.BuildCmykRamp` (keys on `TintRamp`), exercised by `CorpusRenderHashTests`. The **only** true claim is narrower: **no *corpus* render-hash digest moves.** That held because the 51-fixture GWG corpus happens not to contain any of three input shapes that these corrections do change the classification/ramp for — a property of that corpus, measured in Task 0, not a property of the change. Those three shapes (and the interim routed→flattened window one of them opens until Pass 2b lands `ProcessChannel` routing) are recorded in the design doc's Gaps section and in `Docs/colour/rendering-conformance.md` next to G-4, so a future moved digest is attributed rather than hunted. The corpus baseline must still not move by a single digest — see the measurement in Task 0 that establishes why *that* is a stronger claim than the design originally predicted.
