...
🔧 PLANNED (behaviour designed and cited but not yet landed/tested — a forward-declared row for a
phase whose code has not shipped; see the §14.11.5/§8.6.5 sections below for the convention and its
scope note. Every 🔧 PLANNED row must graduate to ✅/⚠️/❌ once its phase lands — it is not a
permanent status).
...
>
> **Status column legend for this slice:** rows 7-1..7-4, 7-10 and 6-1 are `✅` (7-1..7-4 and 6-1
> landed Phase A+B, 2026-08-01; 7-10 landed Phase C, 2026-08-01 — tested, see below). Row 7-6 is
> `⚠️` — Phase C (Task 4b) landed and tested a real leg of this row (Indexed-over-ICCBased *images*,
> palette-level conversion) but not the row's full scope (see the row for the remaining piece). The
> remaining `GAP` rows (6-3, 7-5, 7-7, 7-8, 7-9, 7-11) are deliberate scoped-out gaps with no phase
> currently assigned. `✅`/`⚠️`/`␀` keep their normal meanings for rows describing already-existing
> behaviour.
...
| # | Normative statement | Class | Status | Implementation / note |
|---|---|---|---|---|
| 6-1 | Table 401/402 `DestOutputProfile`: "the 'to CIE' (AToB) information may optionally be used to remap source colour values to some other destination colour space, such as **for screen preview or hardcopy proofing**" (ISO 32000-2:2020 EC2 §14.11.5, p.822/839; PDF 32000-1:2008 §14.11.5 Table 365, p.641, identical wording) | L | ✅ | **Proof destination = first output intent with a CMYK `DestOutputProfile`.** The clause is permissive (`may`), so choosing to exercise it for proofing is a design decision under this document's L class, the same way row 5-14 records the spot-blend latitude as a decision rather than a compliance question. `ProofCmykResolver.ResolveDestination` (`PdfLibrary.Rendering.Icc`) picks the destination: the first output intent whose embedded profile decodes to `OutputIntentColorSpace.Cmyk` (`OutputIntentDescriptor.ColorSpace`/`HasDestProfile`, `PdfLibrary/Document/OutputIntents.cs`), else `CmykProfileProvider.Default` — **`/S` (`OutputIntentSubtype`) is not inspected**, so a `GTS_PDFA1` (or any other subtype) intent with a CMYK `DestOutputProfile` qualifies exactly the same as `GTS_PDFX`; the Phase C test fixtures rely on this (they use `/S /GTS_PDFA1`). Not filtering by subtype is itself a deliberate reading of the permissive clause, noted here rather than left as an accidental gap. The transform itself uses the profile's **BToA leg** ("from CIE"), matching the clause's own framing of `DestOutputProfile` as "the transformation from the PDF document's source colours to output device colourants" — the forward (AToB) direction is reserved for reading an ICCBased *source* space (row 7-1 below), not for the proof destination. Landed in the B-2 merged Phase A+B (2026-08-01); tested by engine `ProofCmykResolverTests` (fallback/destination-selection cases) and the Pellucid Ghent scoreboard, which now proofs through the document's OutputIntent. |
...
| # | Normative statement | Class | Status | Implementation / note |
|---|---|---|---|---|
| 7-1 | §8.6.5.5 ICCBased colour spaces: "the colour space is being used as a source colour space, only the 'to CIE' profile information (**AToB** in ICC terminology) shall be used; the 'from CIE' (BToA) information shall be ignored when present" (ISO 32000-2 §8.6.5.5, p.193/206; identical in PDF 32000-1:2008 §8.6.5.5, p.151) | N | ✅ | **ICCBased fills/strokes, N=3 (RGB) and N=4 (CMYK).** `ProofCmykResolver.TryIccToProofCmyk` builds a two-profile `IccTransform` (ICCSharp) that reads the source ICCBased profile's AToB leg and the destination profile's BToA leg (row 6-1's `ProofCmykResolver` destination) in one composed transform back out to CMYK. `PdfGraphicsState.ResolvedFillProofCmyk`/`ResolvedStrokeProofCmyk` hold the resolved value. Rendering intent: RelativeColorimetric, no black-point compensation, for the whole phase (row 7-10). Landed in the B-2 merged Phase A+B (2026-08-01); tested by engine `ProofCmykResolverTests`, `ProofCmykStateTests`, `IccPcsLabTransformTests` and Pellucid `InkDeciderProofCmykTests`. |
| 7-2 | §8.6.5.4 Lab colour spaces: L* range is "0 to 100" (fixed by the space, not by `/Range`); a*/b* range is the dictionary's `/Range` entry, "[Component] values falling outside the specified range shall be adjusted to the nearest valid value without error indication" (ISO 32000-2 §8.6.5.4, p.190/204; PDF 32000-1:2008 §8.6.5.4, p.148) | N | ✅ | **Lab fills.** Same `IccPcsLabTransform` destination leg as row 7-1, since Lab is already the PCS the transform expects — no AToB source leg is needed for a Lab source space, only the destination profile's BToA leg and the L*/a*/b* clamp above (existing `/Range` clamping logic, unaffected by this phase). Landed in the B-2 merged Phase A+B (2026-08-01); tested by engine `ProofCmykResolverTests`, `ProofCmykStateTests`, `IccPcsLabTransformTests` and Pellucid `InkDeciderProofCmykTests`. |
| 7-3 | Same clause as 7-1, applied to image sample data rather than `scn`/`SCN` operands — §8.9.5.2's "Colour space" entry on an image XObject reuses §8.6.5.5 unchanged; no separate normative text for images. | N | ✅ | **ICCBased images, 8-bit, N=3/N=4.** `ImageCommand.ProofCmyk` carries the resolved per-image proof value alongside the existing device conversion, produced by `ProofCmykResolver.TryIccImageToProofCmyk` via the same two-profile `IccTransform` as row 7-1, at image-sample granularity rather than per-operand. Landed in the B-2 merged Phase A+B (2026-08-01); tested by engine `ImageProofCmykTests` and Pellucid `CmykImageProofPlaneTests`. |
| 7-4 | Same clause as 7-1, applied to JPEG 2000 image data whose PDF-level `/ColorSpace` is a 3-channel ICCBased entry overriding the codestream's own colour space (§7.4.9's `/JPXDecode` filter defers colour-space authority to the PDF-level entry when present) | N | ✅ | **JPX with a 3-channel PDF-level ICC override.** Same conversion as row 7-3; the JPX-specific part is only which colour space wins (PDF-level, per §7.4.9), not the conversion itself. Landed in the B-2 merged Phase A+B (2026-08-01); tested by engine `ImageProofCmykTests` and Pellucid `CmykImageProofPlaneTests`. |
...
| 7-6 | §8.6.6.3 Indexed colour spaces: "base parameter … shall be any device or CIE-based colour space … If the base colour space is a CIE-based ABC space such as a CalRGB or Lab space, the values shall be interpreted as A, B, and C components" — an ICCBased base is likewise CIE-based and in scope (ISO 32000-2 §8.6.6.3, p.199/214) | N | ⚠️ | **Non-JPX Indexed-over-ICCBased — images landed, Task 4b (2026-08-01, engine `892070d`).** An `/Indexed` colour space whose `base` is `[/ICCBased …]`, used as an **image** XObject's `/ColorSpace`, now gets a proof-CMYK plane: `ProofCmykResolver.TryIccImageToProofCmyk` converts the resolved palette (not each expanded pixel) through row 7-1's transform once, then expands per-pixel through the small proof palette — 8-bit indices only. This closed the two `13.0` scoreboard image patches (Indexed-over-ICCBased with `/Intent /Perceptual`); tested by engine `ImageProofCmykTests` (proof plane + palette-expansion correctness) and Pellucid `CmykImageProofPlaneTests`. **Still open:** a non-image `/Indexed` operand reached via `scn`/`SCN` (fills/strokes) does not resolve its base-space triple through row 7-1's transform — the `ProofCmyk` plumbing this row originally described reaches image samples now but not a fill/stroke-time Indexed lookup; and indexed images at other than 8 bits per component are untested. JPX remains excluded from this row's scope for the reason already stated (JPX carries its own indexed/palette handling upstream, per row 7-4). |
...
| 7-10 | §11.7.5.3 (EC2; §11.4.5 in pre-EC2/32000-1 numbering) Rendering intent and colour conversions: "the rendering intent used shall be the current rendering intent in effect in the graphics state at the time of the painting operation" — a per-object parameter, set by the `ri` operator (§8.4.5 Table 57) | N | ✅ | **Landed, Phase C (2026-08-01; engine `bb220b4`, `60b95fa`, `89aec05`, `d00a37b`, `892070d`).** The proof-CMYK leg (`ProofCmykResolver`) now takes a per-call rendering-intent parameter mapped from the graphics state's current `ri` value (vector: `ColorSpaceResolver` ICCBased + Lab legs; images: `image.Intent ?? state.RenderingIntent`, mirroring the sRGB leg's existing precedence). PDF name → ICC intent mapping: Perceptual→Perceptual, Saturation→Saturation, AbsoluteColorimetric→AbsoluteColorimetric, anything else (incl. absent and RelativeColorimetric) → RelativeColorimetric. Transform caches key on intent (`IccColorConverter._cache`'s existing `(Stream, Bpc, Intent)` key is the precedent). A same-object `ri`/ExtGState `/RI` arriving *after* a colour operator now re-triggers resolution via `OnColorChanged` — §11.7.5.3's own wording ("current rendering intent … at the time of the painting operation") makes intent a painting-time, not colour-set-time, parameter; this is what closed the `22.1` scoreboard panel, whose fixture sets `sc` before `ri`. Tested by engine `ProofCmykResolverTests` (per-intent ICC/Lab difference, per-intent cache, unknown-intent default), `ProofCmykStateTests` (`ri` before/after a colour operator, ExtGState `/RI` retroactive) and `ImageProofCmykTests` (`/Intent /Perceptual` difference); intent-differentiation tests use an `RSWOP.icm` OutputIntent fixture with a skip-if-absent guard, since the bundled SWOP_TR003 profile has byte-identical intent LUTs. Scoreboard evidence: `13.0` and `22.1` PASS (board 41/48, 2026-08-01). Scoped out, documented not approximated: black-point compensation stays OFF on the proof leg (the sRGB leg honours state BPC; the proof leg does not); `IccPcsLabTransform` has no AbsoluteColorimetric media-white scaling (Lab-source AbsCol behaves as relative apart from table choice); the CMYK→display simulation (`DeviceCmykConverter`) keeps a single global intent — §11.7.5.3's per-object requirement governs source→proof conversion, not the proof-to-monitor leg; global *display*-intent selection remains a separate, unimplemented concern. |
...
its own header. With Phase C landing 7-10 (2026-08-01), no row in this section is `🔧 PLANNED` any
longer, so a B-2-scoped score is now meaningful under this document's own rule (count only rows a test
has actually exercised) rather than premature:

| §14.11.5 + §8.6.5/§11.4.5 (rows 6-1..7-11) | Count |
|---|--:|
| Normative (**N**) | 12 |
| — ✅ conformant with a test | 5 (7-1, 7-2, 7-3, 7-4, 7-10) |
| — ⚠️ conformant, partial/untested | 1 (7-6 — images landed and tested, fills/strokes and non-8-bit not) |
| — GAP (deliberate, unscheduled) | 6 (6-3, 7-5, 7-7, 7-8, 7-9, 7-11) |
| Latitude (**L**) | 1 |
| — ✅ | 1 (6-1) |
| Device-policy (**D**) | 1 |
| — ✅ (existing) | 1 (6-2) |

No row in this section is `❌` or `␀`. This score is scoped to rows 6-1 through 7-11 only and is not
combined with the slice-1 (Separation/DeviceN) table above.
