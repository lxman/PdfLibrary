...
- **Move the `try/catch` inside the per-glyph loop** (or have the fallback skip the embedded re-attempt). Today a per-glyph exception after ≥1 glyph emitted abandons the run, returns false, and the `DrawText` fallback re-renders the same embedded font → prefix glyphs double-struck. Pre-existing (the original had the same partial-emit→catch→fallback), corpus-never-triggered, but worth hardening. Add a debug-level log in the catch so genuine bugs stop hiding as silent "non-embedded" fallbacks.
- **Consider hoisting `GlyphPathService` to document/session scope.** It's currently `new`'d per `PdfRenderer` (per render). Thread-safety is fine (per-renderer `ConcurrentDictionary` + `GetOrAdd`), but cross-page glyph reuse shrank vs. the original's process-static cache — multi-page documents re-convert outlines per renderer.
- Broaden `CoreTextRendererTests` (currently embedded-fill only, one TrueType font): add stroke, faux-bold, em-dash, invisible mode, Type0-CID, Type1, bare CFF, and the false-return path. Confirm the 100-test render corpus actually includes Type1, bare CFF, and Type0 CID-keyed (incl. CFF descendant) fonts.
