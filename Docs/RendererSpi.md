...
> **But scalar measures are *not* pre-baked.** `LineWidth`, `DashPattern`/`DashPhase`, and any other length carried on `PdfGraphicsState` are in PDF *user space* — the CTM is **not** applied to them. Since the path *coordinates* are CTM-baked, a stroke width must be multiplied by the CTM's linear scale factor, `sqrt(|Ctm.M11·Ctm.M22 − Ctm.M12·Ctm.M21|)`, before you emit it — otherwise strokes drawn inside a `cm`-scaled coordinate system (figures, logos) come out the wrong thickness. The SVG reference target demonstrates this in `StrokePath`, scaling `LineWidth`, `DashPattern`, and `DashPhase` by that factor. This is an easy step to miss.
