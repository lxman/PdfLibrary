...
{
    /// <summary>
    /// MQ probability-estimation state machine (ISO/IEC 15444-1 Table C.2).
    ///
    /// Identical to the table used by JBIG2 (T.88 Table E.1) — the two
    /// codecs share the same arithmetic coder. The values are duplicated
    /// here rather than referenced from Jbig2Decoder so the codecs stay
    /// independently shippable.
    ///
    /// Indexed 0..46. Each row describes one Qe state:
    ///  - <see cref="Qe"/>      — LPS sub-interval size (15-bit fixed-point)
    ///  - <see cref="NMPS"/>    — next index after MPS renormalisation
    ///  - <see cref="NLPS"/>    — next index after LPS renormalisation
    ///  - <see cref="Switch"/>  — if true, MPS sense flips on LPS renormalisation
    /// </summary>
