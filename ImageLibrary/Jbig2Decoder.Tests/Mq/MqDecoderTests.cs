...
public class MqDecoderTests
{
    ...
    public void InitDec_PlainBytes_LoadsExpectedRegisters()
    {
        ...
        object d = New([0x12, 0x34, 0x00, 0x00], 0, 4);
        (uint A, uint C, int CT, int BP) s = GetState(d);
    ...
    public void InitDec_FirstByteIs_FF_FollowedByStuffByte_UsesStuffPath()
    {
        ...
        object d = New([0xFF, 0x00, 0x00], 0, 3);
        (uint A, uint C, int CT, int BP) s = GetState(d);
    ...
    public void InitDec_FirstByteIs_FF_FollowedByMarker_FeedsVirtualOnes()
    {
        ...
        object d = New([0xFF, 0xAC, 0x00], 0, 3);
        (uint A, uint C, int CT, int BP) s = GetState(d);
    ...
    public void InitDec_BufferTooShortForSecondByte_DoesNotThrow()
    {
        ...
        object d = New([0x12], 0, 1);
        (uint A, uint C, int CT, int BP) s = GetState(d);

        // First byte 0x12 loaded into Chigh; BYTEIN advances BP to end. Past
        // the buffer the spec feeds a virtual 0xFF sentinel byte (T.88 §E.3.4
        // end-of-buffer fallback) so the running interval keeps progressing.
        // C = ((0x12 << 16) | (0xFF << 8)) << 7 = 0x097F8000.
        Assert.Equal(0x8000u, s.A);
        Assert.Equal(0x097F8000u, s.C);
        Assert.Equal(1, s.CT);
    }
    ...
    public void Decode_RunsWithoutThrow_OnNonTrivialInput()
    {
        ...
        object d = New([0x84, 0xC7, 0x3B, 0xFC, 0xE1, 0xA4, 0x49, 0x00, 0x00], 0, 9);

        byte cx = 0;

        Type t = typeof(QeTable).Assembly.GetType("Jbig2Decoder.Mq.MqDecoder", throwOnError: true)!;
        MethodInfo decode = t.GetMethod("Decode")!;
