...
public class PackedHeaderStreamTests
{
    ...
    public void PpmSlicer_MultipleSegmentsConcatenateInZppmOrder()
    {
        ...
        PpmSegment segLater = Ppm(1, 0xCC, 0, 0, 0, 1, 0xDD);     // tail of tuple 0 + tuple 1 (Nppm=1, byte 0xDD)
        PpmSegment segFirst = Ppm(0, 0, 0, 0, 3, 0xAA, 0xBB);     // start of tuple 0 (Nppm=3, two of three bytes)
    ...
    public void PpmSlicer_NppmStraddlesSegmentBoundary_StillRecoversChunk()
    {
        ...
        PpmSegment seg0 = Ppm(0, 0, 0, 0);              // first 3 bytes of Nppm
        PpmSegment seg1 = Ppm(1, 4, 0xAA, 0xBB, 0xCC, 0xDD); // last Nppm byte (=4) + 4 payload bytes
    ...
    {
        PpmSegment seg = Ppm(0, 0, 0, 0, 1, 0xAA);
        var slicer = new PpmStreamSlicer(new[] { seg });
        _ = slicer.NextTilePartChunk();
        Assert.True(slicer.IsExhausted);
        Assert.Throws<InvalidDataException>(() => slicer.NextTilePartChunk());
    }
    ...
    public void PpmSlicer_TruncatedNppmPayload_Throws()
    {
        ...
        PpmSegment seg = Ppm(0, 0, 0, 0, 4, 0xAA, 0xBB);
        var slicer = new PpmStreamSlicer(new[] { seg });
        Assert.Throws<InvalidDataException>(() => slicer.NextTilePartChunk());
    }
    ...
    {
        PptSegment s1 = Ppt(1, 0xCC, 0xDD);
        PptSegment s0 = Ppt(0, 0xAA, 0xBB);
