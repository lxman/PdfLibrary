...
namespace ImageLibrary.Compression.Ccitt
{
    /// <summary>
    ...
    public class CcittDecoder
    {
        ...
        {
            return DecodeWithConsumed(data, out _);
        }

        /// <summary>
        /// Decodes CCITT compressed data and reports how many input bytes were
        /// consumed. Used by callers (e.g. JBIG2 halftone planes) that pass a
        /// concatenation of multiple EOFB-terminated MMR streams in a single buffer
        /// and need to advance plane-by-plane.
        ///
        /// On Group-4 input with <see cref="CcittOptions.EndOfBlock"/> enabled,
        /// after the height rows are decoded, the decoder peeks at the next 24
        /// bits and consumes them if they form an EOFB marker (two back-to-back
        /// EOL codes), then rounds the consumed-bit count up to the next byte.
        /// </summary>
        public byte[] DecodeWithConsumed(byte[] data, out int consumedBytes)
        {
            ...
            if (_options.EndOfBlock && _options.Group == CcittGroup.Group4)
            {
                ...
                //                                       = 0x001001 in 24 bits.
                if (reader.BitsRemaining >= 24)
                {
                    int marker = reader.PeekBits(24);
                    if (marker == 0x001001)
                        reader.SkipBits(24);
                }
            }

            // Round up to the next byte boundary to mirror jbig2dec's behavior.
            int finalBits = reader.Position;
            consumedBytes = (finalBits + 7) / 8;
            if (consumedBytes > data.Length) consumedBytes = data.Length;
