...
using ImageLibrary.Compression.Lzw;

namespace ImageLibrary.Container.Tiff;
...
public static class TiffDecoder
{
    ...
    private static byte[] DecompressCcitt(byte[] data, TiffCompression compression, int width, int height, Dictionary<TiffTag, object> tags)
    {
        ...
        return Ccitt.Decompress(processedData, options);
    }
    ...
    {
        // TIFF with JPEG compression is unsupported during the JPEG-decoder rewrite.
        // The previous implementation depended on ImageLibrary.Jpeg, which has been removed.
        // Re-wire this path once the replacement decoder lands.
