...
public class ParityReportTests(ITestOutputHelper output)
{
    ...
    private static readonly IReadOnlyDictionary<ConformanceProfile, int> AgreementFloor =
        ...
        {
            [ConformanceProfile.PdfA2b] = 933,   // +5 prohibited-xobject (6.2.9 → 5/5) +3 image-dictionary (6.2.8 → 3/4; the 4th is an inline image needing content-stream parsing) +4 permissions (6.1.12 → 4/4: /Perms keys + signature-reference Digest keys under DocMDP), all 0 FP. Ratchets to the current verified agreement (the earlier 899 lagged the 921 baseline). Standing −1 note: the 6.2.11.5 width check stays dropped on CIDFontType0/CFF fonts (CFF advance extraction false-positives on conformant reference files, PDFUA-Ref-2-08 — FP-safety outweighs one corpus detection)
