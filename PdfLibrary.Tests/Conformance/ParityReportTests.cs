...
public class ParityReportTests(ITestOutputHelper output)
{
    ...
    private static readonly IReadOnlyDictionary<ConformanceProfile, int> AgreementFloor =
        new Dictionary<ConformanceProfile, int>
        {
            ...
            [ConformanceProfile.PdfUA1] = 277,   // +2 from widening embedded-file to UA-1 7.11 (non-empty /F,/UF → 2/6; the other 4 are reached via file-attachment annotations / /AF, needing broader filespec discovery). Ratchets to the current verified agreement (the earlier 253 lagged the 275 baseline: slice-21 annotation rules + the incremental-update obj-stream resolution fix)
        };
