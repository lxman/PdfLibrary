...
/// </summary>
public class EmbeddedFontIntegrationTests(ITestOutputHelper output)
{
    [Fact]
    public void ExtractEmbeddedFonts_SimplePdf_FindsFonts()
    {
        ...
        foreach (string pdfPath in testPdfs)
        {
            ...
            {
                output.WriteLine($"Skipping {Path.GetFileName(pdfPath)} - file not found");
                continue;
            }
            ...
            try
            {
                ...
                output.WriteLine($"Pages: {pageCount}");
                ...
                for (var pageNum = 0; pageNum < pageCount; pageNum++)
                {
                    ...
                    output.WriteLine($"  Page {pageNum + 1}: {fontNames.Count} font(s)");
            ...
            {
                output.WriteLine($"Error processing {Path.GetFileName(pdfPath)}: {ex.Message}");
            }
        }
    }
    ...
    public void ParseEmbeddedFont_TrueTypeFont_ParsesTablesCorrectly()
    {
        ...
        for (var pageNum = 0; pageNum < doc.GetPageCount(); pageNum++)
        {
            ...
            foreach (string fontName in fontNames)
            {
                ...
                output.WriteLine($"IsValid: {metrics.IsValid}");

                if (!metrics.IsValid)
                {
                    output.WriteLine("WARNING: Font metrics marked as invalid!");
                    continue;
                }
                ...
                foreach (int charCode in testChars)
                {
                    ...
                    output.WriteLine($"  Char {charCode} ('{c}'): PDF={pdfWidth:F2}, Embedded={embeddedWidth} (scaled={scaledWidth:F2})");
                }
        ...
        {
            output.WriteLine("No TrueType fonts with embedded data found in PDF");
        }
    }
    ...
    public void ParseEmbeddedFont_VerifyHmtxTable_ReturnsConsistentWidths()
    {
        ...
        for (var pageNum = 0; pageNum < doc.GetPageCount(); pageNum++)
        {
            ...
            foreach (string fontName in resources.GetFontNames())
            {
                ...
                output.WriteLine($"Character 'A' (65) width: {width1} (consistent across calls)");
                ...
                output.WriteLine($"Character 'W' (87) width: {widthW}");
                output.WriteLine($"Character 'I' (73) width: {widthI}");
                ...
                {
                    output.WriteLine("Font appears to be proportional (W != I width, both positive)");
                }
                else if (widthW == widthI && widthW > 0)
                {
                    output.WriteLine("Font might be monospace (W == I width)");
                }
                else
                {
                    output.WriteLine($"Font is likely a subset (W={widthW}, I={widthI}) - some characters may not be present");
                }
    ...
    public void ParseEmbeddedFont_VerifyCmapTable_MapsCharactersToGlyphs()
    {
        ...
        for (var pageNum = 0; pageNum < doc.GetPageCount(); pageNum++)
        {
            ...
            foreach (string fontName in resources.GetFontNames())
            {
                ...
                output.WriteLine("Character to glyph mapping:");
                ...
                foreach (int charCode in testChars)
                {
                    ...
                    output.WriteLine($"  '{c}' ({charCode}) -> Glyph {glyphId}");
                    glyphsSeen.Add(glyphId);
                }

                output.WriteLine($"Total unique glyphs: {glyphsSeen.Count}");
                ...
                output.WriteLine($"Non-zero glyphs: {nonZeroGlyphs}");
        ...
        {
            output.WriteLine("No fonts with valid cmap tables found");
        }
    }
