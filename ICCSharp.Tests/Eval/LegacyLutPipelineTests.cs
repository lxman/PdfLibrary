...
public class LegacyLutPipelineTests
{
    ...
    public void Mft1_identity_pipeline_returns_input()
    {
        ...
        var inputTables = new byte[i][];
        for (var c = 0; c < i; c++)
        {
            inputTables[c] = new byte[256];
            for (var k = 0; k < 256; k++) inputTables[c][k] = (byte)k;
        }

        // Identity CLUT: each grid point's output equals the grid coordinate.
        var clut = new byte[g * g * g * o];
        var idx = 0;
        for (var r = 0; r < g; r++)
        for (var gp = 0; gp < g; gp++)
        for (var bp = 0; bp < g; bp++)
        {
            clut[idx++] = (byte)(r * 255);
        ...
        var outputTables = new byte[o][];
        for (var c = 0; c < o; c++)
        {
            outputTables[c] = new byte[256];
            for (var k = 0; k < 256; k++) outputTables[c][k] = (byte)k;
        }
    ...
    public void Mft2_identity_pipeline_returns_input()
    {
        ...
        var inputTables = new ushort[i][];
        for (var c = 0; c < i; c++)
        {
            inputTables[c] = new ushort[n];
            for (var k = 0; k < n; k++) inputTables[c][k] = (ushort)(k * 65535 / (n - 1));
        }

        var clut = new ushort[g * g * g * o];
        var idx = 0;
        for (var r = 0; r < g; r++)
        for (var gp = 0; gp < g; gp++)
        for (var bp = 0; bp < g; bp++)
        ...
        var outputTables = new ushort[o][];
        for (var c = 0; c < o; c++)
        {
            outputTables[c] = new ushort[m];
            for (var k = 0; k < m; k++) outputTables[c][k] = (ushort)(k * 65535 / (m - 1));
        }
